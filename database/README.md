...
| `40_logistics_agent_schema.sql` | 재고·물류 Agent 운영 Exception 표 `logistics_exceptions` 신설 (#628 Commit 2). **기존 표를 안 건드린다** — 새 표 하나뿐이고 `sim_runs` 를 FK 로 가리키기만 한다. 표는 **비어서** 나간다. 🔴 이 표가 없으면 걷기의 물류 점검 두 칸이 `FAILED` 한 줄을 남기고 나머지 열세 칸은 그대로 돈다 (하루가 안 죽는다) | 2026-09-12 · **실 DB 적용 대기** |
...
| `40_logistics_agent_schema.sql` | **물류** | 재고·물류 Agent 의 운영 Exception 표. 결정론 탐지기가 쓰고 (`app/logistics/agent/`), 마스터는 트랜잭션만 진다 (`app/master/inspection.py`). `logistics_action_proposals` 는 여기 없다 — 제안·승인이 서는 판(Commit 5)에 만든다 |
