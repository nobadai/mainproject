...
--
--     신규 구축 · 다른 DB 로 마이그레이션  →  orchestrator_agent_runs.sql
--     운영 중인 기존 DB 에 변경분만 적용    →  이 파일
--
-- 여기 있는 세 변경은 **이미 `orchestrator_agent_runs.sql` 본 DDL 에 접혀 들어갔다**
-- (2026-08-27). 새 DB 는 그 파일 하나로 서므로 이 스크립트를 돌릴 필요가 없다.
--
-- 적용 완료 · 대조 확인 — 본 DDL 로 세운 표와 현재 표가 컬럼 20 · 제약 6 · 인덱스 4
-- 전부 일치함을 실측했다.
-- ══════════════════════════════════════════════════════════════════════════
--
-- ★ 새 표를 만들지 않는다.
