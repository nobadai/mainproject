...
function Value({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <dd className="m-0 break-words text-[13px] text-ink">{value}</dd>
    </div>
  );
}
...
function PartnerList({ facts }: { facts: Facts }) {
  ...
  if (items.length === 0) return <p className="m-0 text-sm text-muted">등록된 거래처가 없습니다.</p>;
  return (
    <section className="mt-3">
      <p className="m-0 mb-2 text-xs font-semibold text-muted">거래처 목록</p>
      ...
      <p className="m-0 mt-2 text-xs text-faint">거래처 이름을 입력하면 상세 정보를 확인할 수 있습니다.</p>
    </section>
  );
}
...
function PartnerDetail({ facts }: { facts: Facts }) {
  ...
  return (
    <section className="mt-3">
      <p className="m-0 mb-2 text-xs font-semibold text-muted">거래처 정보</p>
      <Card>
...
function FinanceRows({ facts, kind }: { facts: Facts; kind: "receivable" | "payable" | "expense" }) {
  ...
  if (items.length === 0) return <p className="m-0 mt-3 text-sm text-muted">{empty}</p>;
  return (
    <section className="mt-3 overflow-x-auto">
      <p className="m-0 mb-2 text-xs font-semibold text-muted">{kind === "receivable" ? "미수금 목록" : kind === "payable" ? "지급 예정 목록" : "비용 내역"}</p>
