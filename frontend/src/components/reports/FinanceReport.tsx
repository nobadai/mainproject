...
export function FinanceReport({ facts }: { facts: ReportFacts }) {
  ...
  return <div className="space-y-5">
    <ReportChrome title="재무 운영 보고서" subtitle="재무 상태와 현금 흐름" facts={facts} page={1}>
      ...
      <Section title="기간 Cash Trend"><div data-report-chart="cash-trend" className="h-64 min-h-0 w-full min-w-0 rounded-lg border border-[#dbe7e0] p-3"><ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}><LineChart data={closings}><XAxis dataKey="close_date" tick={{ fontSize: 14 }} /><YAxis tick={{ fontSize: 14 }} /><Tooltip /><Line type="monotone" dataKey="base_net_cash_krw" stroke="#1d6b48" dot={false} /></LineChart></ResponsiveContainer></div></Section>
      <Section title="주요 상태"><div className="grid grid-cols-2 gap-3">{states.map((state) => <div key={text(state.financing_mode)} className="rounded-lg border border-[#dbe7e0] p-3 text-[18px]"><b>{text(state.financing_mode)}</b><br />운영 여유 {won(state.operating_cash_buffer_krw)} · 차입 {won(state.current_debt_krw)}</div>)}</div></Section>
    </ReportChrome>
