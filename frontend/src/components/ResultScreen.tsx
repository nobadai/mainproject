  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center gap-8 rounded-card bg-accent px-8 py-7 text-on-accent">
        ...
        {j ? (
          <>
            <span className="flex items-center gap-3 rounded-full bg-on-accent px-6 py-3 text-xl2 font-semibold text-accent">
              <span className="size-3 rounded-full bg-current" />
              {j.situation === "stable" ? "안정" : "불확실"}
            </span>
            <span className="text-xl2 font-semibold tracking-tight">
              {vm.scenarios.length}안<small className="ml-2 text-sm2 font-normal opacity-75">제시</small>
            </span>
            <div className="ml-auto flex flex-wrap gap-2">
              {(["quantity", "timing", "mix"] as const).map((a) => {
                ...
                return (
                  <span
                    key={a}
                    className={`rounded-ctl border px-4 py-2 text-sm2 font-semibold ${
                      on
          ...
          <>
            ...
            <div className="ml-auto max-w-[520px] rounded-card border border-on-accent/35 px-4 py-3 text-sm2 leading-relaxed">
      ...
      <div className="mb-7 rounded-card border border-rule bg-surface px-6 py-4 text-md2 text-ink-2">
        <b className="mr-3 font-semibold text-ink">판단 요약</b>
        {vm.reasoning}
      </div>
    </>
  );
...
function GateStrip({ vm }: { vm: ViewModel }) {
  ...
  return (
    <div className="mb-7 grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] overflow-hidden rounded-card border border-rule bg-surface">
      <div className="border-r border-rule px-6 py-5 last:border-r-0">
        ...
        >
          ...
          <span className="mt-1 block text-sm2 font-normal tracking-normal text-ink-3">
            {n(d.current)} → {n(d.predicted)}원/kg
          </span>
        </div>
        <div className="text-sm2 leading-relaxed text-ink-2">{d.say}</div>
        <div className="mt-3 font-mono text-xs2 text-ink-3">{d.ref}</div>
      </div>
      {vm.gates.map((g) => (
        <div key={g.name} className="border-r border-rule px-6 py-5 last:border-r-0">
          ...
          <div className="mt-3 font-mono text-xs2 text-ink-3">{g.ref}</div>
        </div>
      ))}
    </div>
  );
...
export function ResultScreen({
  ...
  return (
    <>
      ...
      <div className="mb-6 flex gap-1 rounded-card border border-rule bg-surface-2 p-1">
        {Object.keys(SCENES).map((d) => (
          <button
            ...
            onClick={() => onFlip(d)}
            className={`flex-1 rounded-ctl px-4 py-3 text-center text-md2 font-normal disabled:opacity-50 ${
              d === vm.asOf
          ...
          >
            {d}
            <span className="mt-1 block text-xs2 font-normal text-ink-3">{SCENES[d].blurb}</span>
            {d === vm.asOf && (
              <span
                className={`mt-1 inline-block rounded-ctl px-2 py-1 text-xs2 font-semibold ${
                  vm.source === "api"
      ...
      {j && j.contextDocs.length > 0 && (
        <div className="mb-7 flex flex-wrap items-center gap-3 rounded-card border border-warn-line bg-warn-soft px-6 py-5">
          <Lbl>참조 문서</Lbl>
          {j.contextDocs.map((doc) => (
            <span
              key={doc}
              className="rounded-ctl border border-warn-line bg-surface px-3 py-1 font-mono text-sm2 font-semibold text-warn"
            >
              {doc}
            </span>
          ))}
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          {vm.scenarios.map((s, i) => (
            <button
              ...
              aria-pressed={i === idx}
              onClick={() => setPick(i)}
              className={`rounded-card border px-6 py-3 text-md2 font-semibold ${
                i === idx
      ...
      <div className="mt-7 overflow-hidden rounded-card border border-rule bg-surface">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(230px,1fr))] gap-px bg-rule">
          <div className="bg-surface px-6 py-5">
            <Lbl>근거 항목</Lbl>
            <div className="num mt-2 text-lg2 font-semibold">
              {vm.evidenceCount === null ? "미노출" : `${vm.evidenceCount}건`}
            </div>
          </div>
          <div className="bg-surface px-6 py-5">
            <Lbl>미수신 데이터</Lbl>
            <div className="num mt-2 text-lg2 font-semibold">{vm.missingData.length}건</div>
          </div>
          <div className="bg-surface px-6 py-5">
            <Lbl>사용 Tool</Lbl>
            <div className="num mt-2 text-lg2 font-semibold">{vm.usedTools.length}개</div>
          </div>
          <div className="bg-surface px-6 py-5">
            <Lbl>실행 식별자</Lbl>
            <div className="num mt-2 text-sm2 font-semibold break-all">{vm.runId}</div>
          </div>
        </div>
        {(vm.missingData.length > 0 || vm.concerns.length > 0 || vm.skippedChecks.length > 0) && (
          <div className="space-y-2 border-t border-rule bg-surface-2 px-6 py-5">
        ...
        <div className="flex flex-wrap gap-6 border-t border-rule bg-surface-2 px-6 py-5">
          <span className="flex items-center gap-3 text-sm2 text-ink-2">
