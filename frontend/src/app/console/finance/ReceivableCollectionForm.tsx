...
export function ReceivableCollectionForm({ simRun, asOf, rows, states, onSaved }: { simRun: string; asOf: string; rows: ReceivableRow[]; states: FinanceStateView[]; onSaved: () => void }) {
  ...
  return <Panel title="수금 기록" subtitle="실제 입금만 기록합니다. 전액 또는 이번에 받은 금액을 선택할 수 있습니다.">
    {open.length === 0 ? <p className="m-0 text-[12px] text-ink2">기준일에 기록할 미수금이 없습니다.</p> : <form className="finance-form grid gap-3 sm:grid-cols-2" onSubmit={submit}>
      ...
      {collectAll && selected && <p className="mb-0 self-end text-[12px] text-ink2">이번 수금액: {moneyWon(selected.outstanding_amount_krw)} (서버가 잔액 기준으로 확정)</p>}
      <label className="sm:col-span-2">입금 근거 자료<input required maxLength={240} value={source} onChange={(e) => setSource(e.target.value)} placeholder="예: 통장거래-2026-0916-001" /><small className="mt-1 block text-[11px] text-ink2">입금 거래번호, 입금표, 확인 메일처럼 실제 수금을 확인할 수 있는 자료를 적어 주세요.</small></label>
      ...
      <button disabled={saving || !receivableId || !mode} className="w-fit rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" style={{ background: "var(--color-brand)" }}>{saving ? "기록 중" : "수금 기록"}</button>
    </form>}
