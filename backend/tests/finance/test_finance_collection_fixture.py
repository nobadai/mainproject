...
from app.finance.collection_fixture import (
    DeterministicCollectionFixtureSource,
    collection_events_for_date,
)
...
def test_returns_only_explicit_events_for_the_requested_date():
    ...
    assert collection_events_for_date(
        sim_run_id="SIM-1",
...
def test_cumulative_target_is_preserved_exactly():
    ...
    got = collection_events_for_date(
        sim_run_id="SIM-1",
        financing_mode="LOAN_BASELINE",
        as_of=date(2026, 1, 15),
        source=source,
...
def test_repeated_lookup_is_deterministic():
    ...
    first = collection_events_for_date(
        sim_run_id="SIM-1",
        financing_mode="LOAN_BASELINE",
        as_of=date(2026, 1, 15),
        source=source,
    )
    second = collection_events_for_date(
        sim_run_id="SIM-1",
        financing_mode="LOAN_BASELINE",
        as_of=date(2026, 1, 15),
        source=source,
...
def test_no_fixture_source_means_no_event_not_auto_generation():
    assert collection_events_for_date(
        sim_run_id="SIM-1",
        financing_mode="LOAN_BASELINE",
        as_of=date(2026, 1, 15),
