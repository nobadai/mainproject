...
def test_UTC_자정_직후여도_KST_날짜가_나온다():
    ...
    got = today_in_seoul(_at(datetime(2026, 9, 8, 0, 30, tzinfo=_UTC)))

    assert got == date(2026, 9, 8), "UTC 자정 직후를 전날로 읽었다"
...
def test_UTC_전날_밤이_KST_로는_오늘이다():
    ...
    """
    got = today_in_seoul(_at(datetime(2026, 9, 7, 23, 0, tzinfo=_UTC)))

    assert got == date(2026, 9, 8), "UTC 밤 시각을 서울 날짜로 안 옮겼다"


def test_KST_자정_직전은_아직_그날이다():
    """반대쪽 끝도 본다 — 한쪽만 맞으면 오프셋을 반대로 더해도 통과한다."""
    got = today_in_seoul(_at(datetime(2026, 9, 8, 23, 59, tzinfo=SEOUL)))

    assert got == date(2026, 9, 8)


def test_KST_자정_직후는_다음_날이다():
    got = today_in_seoul(_at(datetime(2026, 9, 9, 0, 1, tzinfo=SEOUL)))
...
def test_시간대_없는_시각은_거절한다():
    ...
    with pytest.raises(ValueError):
        today_in_seoul(_at(datetime(2026, 9, 8, 0, 30)))  # noqa: DTZ001 — 일부러 naive 다
