...
def test_order_date_는_그_실행의_as_of_다(monkeypatch, 이력, 부서들, 확정):
    ...
    # ★ 벽시계를 읽었다면 오늘이 실린다 — 원 실행일과 오늘이 같아지는 날을 피해
    #   날짜를 고정으로 비교한다 (`DTZ011` 을 피하는 것이기도 하다).
    assert 보낸것.order_date == 원_실행일 != datetime.now(UTC).date(), "벽시계를 읽었다"
