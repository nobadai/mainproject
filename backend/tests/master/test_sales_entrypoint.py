                ...
                "scenarios": [
                    {
                        ...
                        # 🔴 상업조건 둘이 없으면 후보가 presented 에 안 오른다
                        #    (2026-09-08 계약 · `CandidateVerdict.missing_terms`).
...
def test_판매_예산_기본값은_25_다():
    """🔴 **매입 12 를 복사하면 요청이 골격을 이긴다** (설계 §3).

    골격에 `SALES_BUDGET = 25` 가 있어도 요청이 12 를 들고 오면 **그 값이 이긴다.**
    소진은 `SL5_BUDGET_EXHAUSTED` 로 남는데, 그건 오류가 아니라 *"판단이 안 끝났다"* 라
    화면이 조용히 덜 본 결과를 보여준다.

    ★★ **16 에서 25 로 올렸다** (2026-09-10 · 추가공급 라우팅 개방). 매입
      `SUPPLY_CAPACITY_QUERY` 가 회차당 최대 3(품목 3) 붙는다.

    ```text
    후보 3 · 되먹임 2회  물류 1 + 판매 3 + 재무 9 + 매입 9 = 22
    +2 물류 재조회 · +1 S-2 재시도                          = 25
    ```
    """
    assert _request().budget == 25
