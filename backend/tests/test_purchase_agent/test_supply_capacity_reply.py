...
🔴 **봉투는 아직 이 mode 를 안 연다.** `_AGENT_MODES["purchase"]` 에 없어서 정상
  경로로는 요청을 만들 수조차 없다 — 마스터가 라우팅을 채우는 날 열린다. 그래서
  여기서는 봉투를 열어 두고 잰다 (그날의 재현).
"""
...
def test_봉투는_아직_이_mode_를_안_연다():
    """🔴 **우리 목록만 열려 있다.** 그것이 마스터에게 보내는 신호다.

    이 검사가 깨지는 날은 마스터가 라우팅을 채운 날이고, 그때는 이 파일의
    `opened` 픽스처가 필요 없어진다 — 그날 이 검사를 지운다.
    """
    assert "SUPPLY_CAPACITY_QUERY" not in envelope._AGENT_MODES["purchase"]
