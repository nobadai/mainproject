...
def test_ci_width_boundary_is_explicit() -> None:
    ...
    통째로 이 값 위에 있다. **셋이 같은 값인 것도 시연값이라서다** — ``s`` 가 오면
    셋은 갈라진다.

    🔴 **순서를 잘못 옮겨 적었다** (2026-09-06 정정). 여기 *"``#67`` 이 확정한 순서는
      양파 < 무 < 배추"* 라고 적혀 있었는데, 그건 그 표의 **"10번 중 몇 번 버퍼가
      깨지나"** 순서다. **임계 순서는 양파 0.1032 < 배추 0.1048 < 무 0.1078 로
      무·배추가 뒤바뀐다** (``δ = 4.71 ÷ s`` 라 ``s`` 가 작을수록 임계가 느슨하다).
      값이 셋 다 ``0.08`` 이라 지금 판정은 안 바뀐다 — 자세한 표는
      ``constraints.yaml`` §situation 주석에 있다.
