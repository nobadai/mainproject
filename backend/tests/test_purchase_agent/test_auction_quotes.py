...
def test_the_connection_carries_a_connect_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    🟢 **화면은 이제 안 멈춘다** (2026-09-11 · ``#580`` 머지). 이 절이 **두 번**
      틀렸다 — 처음엔 *"프론트 15초 폴백이 막는다"* (없는 것을 있다고), 다음엔
      *"프론트에 ``AbortController`` 가 0건"* (생긴 것을 없다고). 지금 프론트는
      ``lib/api.ts`` 읽기 20초 · 실행 900초, ``lib/screen.ts`` 읽기 20초로 끊는다.
      ★ 남의 파일의 **수를 옮겨 적었기 때문**이다. 그래서 여기서는 수를 안 적고
    ...
    🔴 **값을 못 박지 않고 범위로 잠근다.** 재려는 것은 *"5냐"* 가 아니라
      *"무제한이 아니고, 사람이 기다릴 만한 시간 안에 끊나"* 다. 못 박으면 5→4 로
      바꾸는 날 의미 없이 깨지고, 그때 검사를 지우게 된다.

    🟢 **상한 ``15`` 가 근거를 되찾았다** (2026-09-11 · ``#580``). ~~원래
      *"프론트 15초보다 먼저"* 였는데 그 15초가 없다~~ 고 적어 두었는데, 프론트가
      **읽기 20초**로 끊기 시작했다. 그러니 *"백엔드가 프론트보다 먼저 포기한다"*
      가 다시 참이고, ``15 < 20`` 이라 이 상한이 그 순서를 여유까지 두고 잠근다.
      ★ **그 순서라야 사유가 안 덮인다** — 프론트가 먼저 끊으면 화면에 *"서버가
        느립니다"* 만 뜨고, 백엔드가 낸 *"DB 에 못 닿았다"* 가 그 아래 묻힌다.
      ⚠️ **상한은 15 그대로 둔다.** 20 으로 올리면 프론트와 같아져 순서가 안 잠긴다.
        ★ 이 상한이 무는 것은 **상수 쪽 변이**다 — ``CONNECT_TIMEOUT_SECONDS`` 를
          20 으로 늘리면 이 검사가 죽고, 상한을 걷으면 그 변이가 안 물린다
