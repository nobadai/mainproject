...
def test_the_connection_carries_a_connect_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    🔴 **화면도 같이 멈춘다** (2026-09-07 정정). 전에 *"화면은 프론트 15초 폴백이
      막지만 백엔드 워커가 물린다"* 고 적었는데 **앞 절이 틀렸다** —
      ``frontend/src/lib/api.ts`` 116줄에 ``AbortController`` · ``setTimeout`` ·
      ``signal`` 이 **0건**이고 ``fetch`` 는 기본 타임아웃이 없다. ``#81`` 본문의
      *"임시 방어 — 프론트는 이미 막았다"* 에서 옮겨온 문장이고, 원문이 틀렸다.
      워커가 물리는 것은 그대로다.
    ...
    ⚠️ **상한 ``15`` 는 근거를 잃었다** (2026-09-07). 원래 *"프론트 15초보다 먼저"*
      였는데 그 15초가 없다. 그래도 **남긴다** — 걷으면 값을 20 으로 늘리는 변이가
      안 물려 검사가 *"인자가 나가나"* 만 재게 된다 (실측: 20 → 1 failed,
      상한 없으면 통과). 지금은 **근거 없는 울타리**이고, 제 값은 ``#81`` 의
      ⓐ(각자)/ⓑ(공통 헬퍼) 결정과 함께 정할 자리다.
    ...
    # ⚠️ 상한 15 의 근거는 위 docstring 에 있다 — 프론트가 안 끊으므로 "프론트보다
    #    먼저" 가 아니라 지금은 근거 없는 울타리다. 값을 바꾸려면 #81 과 함께 본다.
