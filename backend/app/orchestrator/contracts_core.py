# ─────────────────────────────────────────────────────────────────────────────
# STATUS: 공용 계약 (2026-08-26)
#   마스터·검증·앞으로의 재무/물류 어댑터가 전부 여기서 타입을 가져온다.
#   `app/master/envelope.py` 가 Evidence·Verdict·RuntimeStatus·SuggestedAdjustment 를 쓴다.
#   🔴 위치가 잘못됐다 — 공용 계약이 한 파트 폴더에 있어 마스터가 "오케"를 임포트하는 모양이다.
#   → 발표 후 1단계에서 `app/contracts/` 로 승격. 임포트 경로만 바뀌는 기계적 변경.
# ─────────────────────────────────────────────────────────────────────────────
"""
