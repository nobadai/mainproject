...
def detect_variant_collapse(
    ...
    🔴 **`①` 은 2안에서만 종착역이다 — 3안 이상은 `②` 까지 본다** (매입 결정 2026-09-07).

      전에는 지문이 전부 다르면 스프레드가 임계 아래여도 그대로 통과했다. 매입 실측
      `cap=4.5M` 이 그 구멍을 그대로 보여 준다.

      ```text
      보수  2,571  1회차          지문 A
      기본  2,727  1회차          지문 B    ← 총량이 156kg(5.7%) 차이
      공격  2,727  2회차 분할     지문 C    ← 총량은 기본과 같고 분할만 다르다

      지문 3/3 이라 ① 에서 통과하고, 스프레드 0.057 을 아무도 안 본다
      ```

      ★ 매입 근거 — *"2안은 「둘 중 하나」라 분할 차이도 선택지가 되지만, 3안에서 둘이
        5% 차이면 셋째가 있어도 고를 게 없다.* **안의 개수가 「선택지」의 뜻을 바꾼다.**"

      **그래서 2안(`len(feasible) == 2`)은 그대로 둔다.** `:690` 의 5,449kg 예시 —
      오늘 전량 vs 오늘 2,725 / D+3 2,724 — 는 여전히 `①` 에서 통과한다. 둘 중 하나를
      고르는 자리에서는 분할 구조가 그 자체로 선택지이기 때문이다.

      ⚠️ **대가는 매입이 스스로 짚었다.** *"`len(feasible)` 을 분기 조건으로 쓰면
        「안이 몇 개냐」가 판정 규칙을 바꾸는 셈이라 그 자체가 근거를 요구한다."*
        근거는 위 한 문단이 전부이고, 늘리지 않는다.

      ⚠️ **발표(2026-09-13)까지 이 분기는 한 번도 안 돈다.** 실데이터는 전부
        `uncertain` 이고(매입 전수 84조합 stable 0건), `classify_situation.py:121` 이
        uncertain 이면 공격안을 빼므로 **2안만 나온다.** 그래도 넣는 것은 결정이 났고
        2안 경로에 영향이 0이기 때문이다 — 발표 뒤 stable 이 나오는 날 살아난다.
    """
    ...
    # ★ 안이 셋 이상이면 ① 이 종착역이 아니다 (매입 결정 2026-09-07)
    many_variants = len(feasible) >= 3

    # ① 지문이 **전부** 다르면 살아 있는 선택지다 (분할 구조만 달라도 다른 선택이다)
    #   ⚠️ 2안에서만이다 — 3안 이상은 지문이 다 달라도 ② 로 내려간다
    if len(distinct) == len(feasible) and not many_variants:
        return False
