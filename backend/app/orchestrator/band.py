...
def detect_variant_collapse(
    ...
    ⚠️ **지문이 전부 다르면 스프레드가 임계 아래여도 통과한다.** 일부러 그렇게 둔다.

      ```text
      보수 2,571 · 기본 2,727 · 공격 2,727(2회차)   지문 3/3 · 스프레드 0.057
      → 통과한다 (임계 0.15 아래인데도)
      ```

      **분할 구조가 선택지를 만들기 때문이다** — `①` 의 근거가 그것이고, 총량이 붙어
      있어도 오늘 전량과 오늘 반·D+6 반은 현금흐름과 로트 나이가 다르다.

      🔴 **다만 사람이 받는 화면에서는 이것도 "고를 게 없어" 보인다** (매입 지적
        2026-09-06). *"총량이 붙었는데 분할만 다른 안을 선택지로 볼 것인가"* 는
        `:690` 이 2안 기준으로 정한 **도메인 판단**이고, 3안에도 그대로 적용할지는
        매입·팀이 정할 자리라 여기서 안 뒤집는다. 뒤집으려면 `②` 를 `①` 뒤에도
        돌려야 하는데, 그러면 `:690` 의 2안 예시가 같이 걸린다.
    """
