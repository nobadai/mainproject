...
🔴 **되돌아오는 간선은 없고, 앞으로도 안 만든다** (2026-09-09 · E3-6 결정).

상세설계 §6 이 *"quantity 위반 → ③부터 재실행 · sourcing 위반 → ⑤만 · timing 위반 →
④만"* 이라는 축별 재진입을 그려 뒀는데, **재보니 상대가 없다.**

```text
마스터가 통째로 다시 부른다   flow.py  runner.call("purchase", "GENERATE_SCENARIOS", …)
                              → 그래프는 ①부터 새로 돈다. 재진입은 마스터가 소유한다
보내는 쪽이 축을 안 보낸다     violated_axis · constraint · keep 은 저장소 어디에도 없다
                              오는 것은 "이 안의 이 항목을 이 값 이하로" 뿐이다
```

★ 그 설계는 계약 요구가 아니라 **채점 대비 심화**였다 (§0 멘토링 반영 표 —
