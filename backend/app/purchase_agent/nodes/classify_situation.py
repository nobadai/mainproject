...
def classify_situation(state: PurchaseAgentState) -> dict[str, Any]:
    ...
    🔴 **이 자리에 셋 다 "읽고 싶어도 못 읽는다" 고 적었었다 (2026-09-03 정정).**
      ...
      **``DailyPoint`` 를 근거로 삼은 것이 틀렸다.** 그 모델(``app/ml/schemas.py``)
      ...
      ``use_recommended`` 처리는 IO명세 §8 이 **#57** 로 배정해 뒀었다 —
      **2026-09-03 에 #213 으로 뺐다.** 넷이 같은 층이 아니고 선행도 갈려서, 묶어두면
      #57 본문(rise_rate 분모)이 플래그 배선을 기다리게 된다.

      ⚠️ **#57 은 본문이 아니라 코멘트에 있었다.** 본문(*"rise_rate 분모를 당일 시세
        조회로 전환"*)에는 ``use_recommended`` 가 **0건**이라, 제목만 보면 딴 이슈로
        읽힌다 — **분리한 이유가 그것이다.** 배정이 있던 코멘트 넷::

            2026-08-27 08:45   ML 실계약 확정 — 마지막 줄이 "착수 시 수신 검증에
                               is_filled(판정일)·use_recommended 처리 포함"
            2026-08-27 09:20   "is_filled 확정 편입 … use_recommended 와 함께 수신 검증에 포함"
            2026-08-27 09:49   지환님 해석 확정 — is_gated=출처 / use_recommended=사용 권고,
                               gate_reason quality→제외 · lead_time→사용
            2026-09-01 04:48   남는 작업 셋 — "is_filled(판정일 복사값) · use_recommended 처리"

      🔴 **이 목록의 앞 두 줄이 틀렸었다 (2026-09-03 정정).** ``05:23`` 을 적었는데 그
        코멘트는 ``price_basis``·``size_class``·``grade`` 동봉 검사 건이고, 인용한
        문장은 ``08:45`` 마지막 줄에 있다. ``08:45`` 를 *"같은 문장"* 으로 따로 세어
        **한 건을 두 번 세고**, 정작 ``09:49`` 는 빠뜨렸다.

        ⚠️ *"코멘트에만 있어서 안 보인다"* 고 쓰는 자리에서 **그 코멘트 좌표를 틀렸다.**
          목록을 옮겨 적을 때 원본과 대조하지 않으면 이렇게 된다.
