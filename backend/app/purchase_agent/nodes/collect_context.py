...
**LLM 자리가 두 곳 있었고, 2026-09-09 에 둘 다 «규칙으로 간다»로 닫았다** (E3-8 · 규칙 6):
...
def select_doc_types(constraints: dict) -> list[str]:
    ...
    🔴 **LLM 자리 ① 이었다. 2026-09-09 에 재보니 «고르는 일»이 없어 안 꽂는다** (E3-8).

        loop_max(3) == len(doc_type_priority)(3)   순서와 무관하게 **셋 다 읽는다**
        ``loops``는 늘 0                            adapter.py · state.py 두 진입점 모두
        ②로 돌아오는 간선이 없다                     graph.py · 되먹임(E3-6)도 미구현
        무·양파는 관측월보 1종뿐                     나머지 회차는 어떤 순서로도 빈 결과

    ★ 그래서 순서가 바꾸는 것은 ``rationale`` **나열 순서 하나**이고, 판정·수량·금액·안
      개수는 안 바뀐다. mock 5앵커에서 ②가 도는 날은 ``2026-09-04`` 하나이며(나머지는
      stable), 운영에서는 ⑥이 ``MockNotAllowed``라 애초에 문서가 0건이다 (E4-1).

    🔴 **``doc_type_priority``가 ``loop_max``보다 길어지는 날 이 판단을 다시 한다.**
      그때 «무엇을 읽을까»가 처음 실재한다 —
      ``test_ordering_is_moot_while_the_list_fits_the_loop_budget`` 가 그날 운다.
    """
...
def is_enough(docs: list[dict]) -> bool:
    ...
    🔴 **LLM 자리 ② 였다. 2026-09-09 에 «규칙으로 간다»로 닫았다** (E3-8).

      ⑤ 등급 조합은 LLM 을 꽂았는데 여기는 안 꽂는 이유가 **재료의 성질**에 있다.
      ⑤는 규칙이 판정을 끝낸 뒤 **라벨만** 넘긴다 (``llm/mix.py`` — 숫자를 안 준다).
      충분성은 문서 **본문**을 봐야 판정되는데 관측월보는 숫자투성이라, 그 방어를
...
def collect_context(state: PurchaseAgentState) -> dict[str, Any]:
    ...
    탈출 조건 **세 가지**. 앞의 둘은 **for 경계로 표현한다** — ``while``로 쓰면 "언젠가
    ...
    ⚠️ 문서가 적어 둔 *"충분 판정 OR loop == 3"*(상세설계 §4-②)은 **낡았다.**
    ``is_enough()``가 항상 ``False``라 "충분 판정" 탈출은 살아 있지 않다.
    2026-09-04에 문서를 정정했다.

    🔴 **귀속을 정정한다 (2026-09-07).** 전에 *"`#230`이 그 판정을 걷었다"*고 적었는데
      틀렸다 — `#230`은 ``self_check``의 ``context_unavailable`` 컷을 고지로 바꾼 판이고
      (`#228`을 마스터가 뒤집은 것), ``is_enough``는 손댄 적이 없다.

    ★ **처음부터 없었다.** ``bbebd8a``(E3-4 최초 구현)가 ``return False``로 넣었고
      그 docstring이 지금과 글자 그대로 같다.
