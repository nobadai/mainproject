...
def leading_excerpt(content: str, max_chars: int) -> str:
    ...
    **잘라내기만 하므로 원문 문자는 변조되지 않는다** — 대조는 어느 경우든 성립한다.
    ...
    excerpt = head + terminator if terminator else content[:max_chars]
    ...
    **``loop_max``는 누적 상한이다.** ``state["context_loop_count"]``에서 이어 세고 남은
    ...
    for _ in range(max(0, loop_max - loops)):
        ...
        for doc in ports.get_context_docs(state["item"], as_of, [doc_type]):
            ...
            seen.add(doc["doc_id"])
            collected.append(
                {**doc, "excerpt": leading_excerpt(doc["content"], excerpt_max_chars)}
            )
