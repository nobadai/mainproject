...
class PurchaseAgentState(TypedDict):
    ...
    #
    # 🔄 **값이 바뀐다 — 물류가 `1` 로 간다고 알렸다** (2026-09-10 저녁). 🔴 다만 **런타임은
    # 아직 `2` 다** (같은 날 실측)::
    #
    #     agent_policy_config.inbound_lead_days        2
    #     봉투 constraints.inventory.inbound_lead_days  544건 중 `2` 541 · `2.0` 3
    #                                                  (`None` 172 는 물류 어댑터 경로가
    #                                                   아닌 실행이다)
    #
    # ★ **그래서 여기에 `1` 을 적어 두지 않는다.** 예고와 수신값이 갈려 있고, 우리가 앞질러
    # 적으면 문면이 실제보다 앞서 간다 — N5 에서 정확히 그 값을 치렀다 (`7` → `0` → `D+0`,
    # 하루에 세 번). **동작은 수신값 그대로**이므로 봉투가 `1` 을 싣는 날 따라간다.
    #
    ...
    #
    # 🟡 **바뀌면 도착일 주말 분포가 절반이 된다** (실측 · 회차 155건 기준). 판정은 안
    # 뒤집힌다 — `D+1` 도 오늘은 아니다::
    #
