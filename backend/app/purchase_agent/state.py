...
class PurchaseAgentState(TypedDict):
    ...
    # ``purchase_payment_days``: N5. **정본은 7** (`FINANCE-DECISION-20260827` · calendar day ·
    # 영업일 보정 없음). 🔴 그런데 **런타임은 0 을 보낸다.**
    #
    ...
    #
    # 🔴 ~~그 0 은 「확정된 0」이다~~ — **우리 판정이 틀렸다** (재무 회신 2026-09-10).
    # 관측은 맞았고 **이름이 틀렸다.** 재무가 정본 소유자로서 답한 것은 이것이다::
    #
    #     "N5=0 을 현재 정본으로 인정하지 않겠습니다"
    #     "정책이 7→0 으로 변경된 게 아니라 런타임/설정 정합성이 갈린 것으로 보겠습니다"
    #     "8/27 의 7일 결정을 폐기하고 D+0 으로 변경한 후속 Finance 결정이 없습니다"
    #
    # ⚠️ **그러니 이 0 은 「복구 대상」이지 「새 정본」이 아니다.** 재무가 정합성을 되돌린다.
    #
    # 🟢 **그래도 폴백을 넣지 않는다 — 근거가 바뀔 뿐 결론은 같다.** 재무 원문이다::
    #
    #     "0 은 미정값이 아니라 0 = 매입 당일 지급 이라는 별도의 유효한 의미를 가진 값입니다"
    #     "0 을 falsy 로 보고 다른 값으로 대체하지 않음 … 은 맞습니다.
    #      이번 건 때문에 매입에서 다시 7 을 하드코딩하거나 fallback 을 넣으실 필요 없습니다"
    #
    # ★ 즉 ``pending_value`` 가 ``is None`` 으로 가르는 것은 그대로 옳다 (규칙 3) —
    # **0 이 정책 확정값이라서가 아니라, 0 이 뜻을 가진 값이라서**다. 🔴 반대쪽도 금지다:
    # 정본이 7 이라고 **여기서 7 을 채우지 않는다.** 수신값을 고쳐 쓰면 우리가 재무의
    # 정합성 문제를 우리 코드 안에 숨기게 되고, 복구가 끝나도 아무도 못 알아챈다.
    #
    # 🟡 **마감이 이 값을 그대로 세지는 않는다** (같은 회신). ``daily_closings``
    # ``purchase_cash_out_krw`` 는 N5 가 아니라 **결정된 ``payment_date`` 에 귀속**되고,
    # 이미 확정된 ``payment_date`` 가 있으면 마감이 N5 로 덮어쓰지 않는다.
    # 그래서 재무는 `#150` 마감 적재 **앞에** N5 정합성 복구를 넣었다.
    #
