...
def _observed_at(
    ...
    🔴 **모수가 지금 하나다 — 시세뿐이다.** 봉투는 *"계산에 쓴 입력이 여럿이면 그중
      ...
      🔴 **그때도 ``max()`` 하나로는 모자란다 — 계약이 한 겹 더 있다.** 봉투 주석
        (``master/envelope.py`` 의 ``AgentReply.observed_at``)이 *"입력이 여럿이면
        **가장 늦은** ``observed_at`` 이고, **하나라도 미지면 ``None``** 이다"* 로 적어 뒀다.
        ⇒ 다섯이 오는 날 쓸 식은 ``max(...)`` 가 아니라 **«하나라도 ``None`` 이면 ``None``,
          아니면 ``max``»** 다. ``max()`` 만 넣으면 **미지를 조용히 건너뛴다.**
