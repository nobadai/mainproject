...
**우리 933원 vs ML 812원** 이다.

🔴 ~~후행 1·2·3·5·7·10 영업일 물량가중 어느 창으로도 재현되지 않았다 (2026-08-31 실측).
ML 내부 시리즈라 원자료만으로 복원할 수 없다~~ — **낡았다** (ML 회신 2026-09-10).
**복원된다.** 그 칸은 시세가 아니라 모델이 출발점으로 쓴 **앵커**다::
...
★ 실 DB 로 재현했다 (2026-09-10 · 3품목 × 170 기준일 = 510 조합 중 **491 일치**). 어긋난
19는 ML 이 규격을 바꾼 08-21~08-27 구간과 09-04 · 09-07 이다.

⚠️ **어제값을 섞기 때문에 어떤 「후행 N일 평균」으로도 안 나온다** — 창만 갈아 끼우며
못 찾은 것이 그래서다. ★ 그리고 **그 7 거래일에는 토요일이 든다.** 영업일로 세면 틀린다:
09-05(토) 626.89 를 빼면 09-09 배추 앵커가 704.69 대신 728.40 이 된다.

★ 그래서 ``rise_rate`` 분모는 **ML 이 예측과 같은 행에 동봉한 ``current_price`` 를 그대로
쓴다** (#57). 🔴 **판단은 그대로이고 이유가 바뀌었다** — 「복원 못 해서 어쩔 수 없이」가
아니라 **``predicted`` 가 그 앵커에서 출발하므로 같은 기준선끼리 비교하는 것이 옳아서**다.
당일 시세를 분모로 넣으면 두 시리즈가 섞여 배추 기준 rise_rate 가 12.2%p 갈린다.
"""
