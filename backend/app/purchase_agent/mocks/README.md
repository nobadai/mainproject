...
`ci_width`를 18일 **전 구간에서 일정하게** 유지한다. 상세설계 §4-①이 "어느 날의
`ci_width`로 판정하는가"를 규정하지 않아서 어느 날을 골라도 판정이 뒤집히지 않게 한 것인데,
**이건 회피지 해결이 아니다** — 실제 예측은 날짜마다 폭이 다르다.
① `classify_situation` 노드를 구현할 때 판정 기준일을 확정해야 한다
(후보: D+14 단일 / 전 구간 최대).
