...
#   고정 순서 5단 루프(제안→제약→조정→검증→승인). **`app/master/flow.py` 가 대체한다.**
#   ⚠️ 지금 지우면 `tests/master/critic/test_critic_v0_4.py` **138건이 함께 죽는다** —
#      그 테스트들이 `run_subcycle`·`run_day`·`build_commitment` 로 검증 대상 시나리오를 만든다.
#   → 마스터 어댑터 3종이 실사용으로 검증된 뒤(발표 후 3단계) 제거하고 138건을 새 경로로 이전.
# ─────────────────────────────────────────────────────────────────────────────
