...
⚠️ **말투는 검사하지 않는다.** 실측에서 모델이 결론을 사용자 명령형으로 되돌려
  *"기본 안으로 진행해 주십시오"* 라고 쓴 적이 있다(시스템이 사용자에게 시킨 꼴).
  프롬프트로 고쳤고 검사는 두지 않았다 — *"확인해 주세요"* 같은 정당한 요청과
  기계적으로 가르는 규칙이 없기 때문이다. **이 모듈이 잠그는 것은 사실이지 문장의
  품질이 아니다.**
"""
