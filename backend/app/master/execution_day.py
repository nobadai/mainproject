...
★ **왜 주말을 거르는가. 토·일에는 ML 예측이 없다.** 실측
  (2026-09-04)으로 ML 예측 기준일 12개가 전부 평일이고 주말 기준일은 0건이다.
  (2026-09-09 재측정: `ml_price_forecasts.base_dt` 요일 분포가 월~금 뿐이다.)

  🔴 **「시장이 안 서서」가 아니다** — 그렇게 적어 왔는데 틀렸다. 토요일은
    `ml_calendar_days.is_open` 이 **참**이다 (2026-01 ~ 09 실측 34일 참 / 3일 거짓).
    **시장은 서는데 예측이 없다.** 매입이 짚어 줬고, 그 문장이 사용자 화면에
    그대로 그려지고 있었다 (`service._not_execution_day_reason`).
