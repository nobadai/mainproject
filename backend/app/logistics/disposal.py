...
🔴 **폐기를 실행하는 도메인 경로는 이 함수 하나뿐이다.** Scheduler 도, Agent 도,
   회전 상태도 이 함수를 **직접** 부르지 않는다. `turnover.py` 는 후보만 계산하고 이
   파일을 임포트하지 않는다 — 그 단방향이 *"저절로 재고가 사라지는 경로"* 를 막는다.

   ⚠️ **규칙 기반 호출은 `auto_maintenance` 하나뿐이다** (2026-09-09). 그 조립 계층은
      아래 셋을 **모두** 확인한 경우에만 이 함수를 부른다.

   ```text
   disposal_candidate == True   근거는 여전히 turnover 것 하나뿐이다
   살아있는 할당이 없다           한 kg 이라도 잡혀 있으면 손대지 않고 건너뛴다
   잔량 전량을 폐기할 수 있다     폐기가능량 == remaining_qty_kg · 수량도 그 전량이다
   ```

      🔴 **그 밖의 자동 폐기는 없다. 자동 부분 폐기도 없다.** 셋 중 하나라도 어긋나면
         그 Lot 은 손대지 않은 채 사람 몫으로 남는다 — 되돌릴 경로가 없기 때문이다.
...
def confirm_disposal(
    ...
    *,
    ...
    🔴 **아무나 자동으로 부르면 안 된다.** Scheduler · Agent · 회전 상태가 이 함수를
       직접 부르지 않는다. 되돌릴 경로가 없어서(`ADJUST_IN` 없음 · 실사 제외) 부르는
       쪽이 **그 판단을 명시적으로 했다는 것**이 안전장치다.

       ★ 일반 경로에서는 사람이 판단한 뒤 부른다. 그 판단을 규칙으로 세운 자리는
         `auto_maintenance` 하나이고, 거기서도 **폐기대기 · 살아있는 할당 없음 ·
         잔량 전량** 셋을 모두 확인할 때만 부른다 (모듈 docstring 참조).
