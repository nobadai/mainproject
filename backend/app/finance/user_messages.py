...
#: 요청에 **어느 실행인지가 실려 오지 않은** 경우.
#:
#: 🔴 **`CONTEXT_UNAVAILABLE` 과 같은 말이 되면 안 된다.** 저 문장은 *"자료를 확인하지
#:    못했다"* 라서 읽는 사람이 자금 자료를 찾으러 간다. 그런데 여기서 벌어진 일은
#:    **묻지를 못한 것**이다 — 자료는 그대로 있고, 어느 실행의 자료를 봐야 하는지가
#:    요청에 없어서 물어볼 수가 없었다. 둘을 같은 문장으로 적으면 없는 자료를
#:    하루 종일 찾게 된다 (실제로 그렇게 나갔다 · 매입 판단 513건).
#:
#: ★ 식별자 `sim_run_id` 는 `missing_data` 가 나른다. 문장에는 담지 않는다 —
#:   여기 규칙 그대로다.
EXECUTION_AXIS_MISSING = (
    "어느 실행의 자금 상황을 봐야 하는지가 요청에 실려 있지 않아 재무 검토를 시작하지 "
    "못했습니다. 자금 자료가 없는 것이 아니라, 요청이 실행을 지정하지 않은 것입니다."
)
