...
FINANCE_EXPLANATIONS: dict[str, str] = {
    ...
    # 🔴 **재무가 답한 것은 "재무상 걸리는 것이 없다" 까지다.** 예전 문장은
    #    "그대로 진행하실 수 있습니다" 였는데, 그건 재무의 답이 아니라 **판매 확정**
    #    처럼 읽힌다. 실제 확정은 사람이 고르고 최종 재검증을 통과해야 나는 것이고,
    #    재무 판정 하나로 열리지 않는다 — 문장이 기계 계약보다 넓게 말하고 있었다.
    "SALES_ACCEPT": (
        "현재 자금 상태와 예상 현금흐름을 기준으로 보면 "
        "제안하신 판매 조건에는 재무상 추가 조정이 필요하지 않습니다."
    ),
    "SALES_CONDITIONAL": (
